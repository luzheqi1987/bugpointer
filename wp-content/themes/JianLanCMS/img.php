...
*/
define ('VERSION', '0.0.0');																		// 版本号
//Load a config file if it exists. Otherwise, use the values below
if( file_exists(dirname(__FILE__) . '/timthumb-config.php'))	require_once('timthumb-config.php');
if(! defined('DEBUG_ON') )					define ('DEBUG_ON', false);								// Enable debug logging to web server error log (STDERR)
if(! defined('DEBUG_LEVEL') )				define ('DEBUG_LEVEL', 1);								// Debug level 1 is less noisy and 3 is the most noisy
if(! defined('MEMORY_LIMIT') )				define ('MEMORY_LIMIT', '30M');							// Set PHP memory limit
if(! defined('BLOCK_EXTERNAL_LEECHERS') ) 	define ('BLOCK_EXTERNAL_LEECHERS', false);				// 防盗链
...
if(! defined('FILE_CACHE_SUFFIX') ) 		define ('FILE_CACHE_SUFFIX', '.img.tds');				// 缓存文件后缀
if(! defined('FILE_CACHE_PREFIX') ) 		define ('FILE_CACHE_PREFIX', 'taodushi');				// 缓存文件前缀
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', './tdsimg');			// 缓存目录
...
<h1>图片缓存发生错误</h1>The following error(s) occured:<br />' . $html . '<br />';
		echo '<br />字符串 : ' . htmlentities ($_SERVER['QUERY_STRING']);
		echo '<br />当前缓存系统版本 : Linux.TaoDuShi.1.3.9</pre>';
	}
	...
	protected function serveExternalImage(){
		...
		if(! $this->getURL($this->src, $tempfile)){
			...
			$this->error("读取错误的远程主机从指定URL。" . $this->lastURLError);
			return false;
		}
	...
	protected function serveCacheFile(){
		...
		if(ftell($fp) != strlen($this->filePrependSecurityBlock) + 6){
			@unlink($this->cachefile);
			return $this->error("缓存图像文件可能已经损坏。");
		}
